...
public class StringUtils extends org.apache.commons.lang.StringUtils{
    ...
    public static Map<String,Object> getInfoMap(String str){
        ...
        for (int i=0;i<temp.length;i++){
            ...
            if (subTemp.length > 1){
                if (StringUtils.isNotBlank(subTemp[0]) && StringUtils.isNotBlank(subTemp[1])){
    ...
    /**
     * 解析前台传入的字符串，如“id1,id2,id3....”,得到userId列表
     */
    public static List<String> getUserIdList(String userParams){
        if (StringUtils.isBlank(userParams)){
            return null;
        }

        List<String> list = new ArrayList<String>();
        String[] subStrings = userParams.split(",");
