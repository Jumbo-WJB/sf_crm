...
function renderPage(a, b, c) {
	...
	function(a) {
		...
		d.addInterceptor(function(a, b, c) {
            ...
            if(a.pageId==5641&&(a.properties.src).indexOf("upload")>0){
                return b.id = a.id, b.setAttribute("ctype", a.type), b.setAttribute("class", "element comp_image editable-image"),
                    /^http.*/.test(a.properties.src) ? b.src = a.properties.src : b.src = PREFIX_URL + a.properties.src, b
            }
			return b.id = a.id, b.setAttribute("ctype", a.type), b.setAttribute("class", "element comp_image editable-image"),
                /^http.*/.test(a.properties.src) ? b.src = a.properties.src : b.src = PREFIX_FILE_HOST + a.properties.src, b
...
! function($) {
	...
	function getRequestUrl() {
		var a;
		//return   ? (a = PREFIX_URL + "index.php?c=scene&a=view&id=" + sceneId, branchid && (a += (/\?/.test(a) ? "&" : "?") + "user=" + branchid)) : a = mobileview ? PREFIX_URL + "index.php?c=scene&a=view&id=" + sceneId : PREFIX_URL + "index.php?c=scene&a=view&id=" + sceneId, a += (/\?/.test(a) ? "&" : "?") + "time=" + (new Date).getTime()
	//	return PREFIX_URL +"/media/mobile/assets/vendor/mygrow/json/cy.json";
        return  VISIT_URL;
	}
	...
	function parseJson(a) {
		document.title = a.obj.name, $("#metaDescription").attr("content", a.obj.name + ", " + a.obj.description + ", 由酷秀免费移动场景应用自营销管家提供技术支持"), $(".scene_title").html(a.obj.name), pageMode = a.obj.pageMode;
		var b = [];
		return a.obj.property && (a.obj.property = JSON.parse(a.obj.property)), b = a.list, b.length <= 0 ? (alert("此场景不存在!"), void(window.location.href = "http://yiqixiu.tongkuvip.com")) : (appendLastPage(a, b), void bindShare(a))
	}
	...
	function appendLastPage(a, b) {
		...
		if (a.obj.createTime > 14165028e5)
			...
			a.obj.property && a.obj.property.bottomLabel && a.obj.property.bottomLabel.id ? $.ajax({
				...
				success: function(c) {
					...
					for (d.length; e < d.length; e++) {
						var f = d[e];
						if (f.sceneId = a.obj.id, f.pageId = b[b.length - 1].id, a.obj.property.bottomLabel.name && a.obj.property.bottomLabel.url && "http://" != a.obj.property.bottomLabel.url) 2 == f.type && f.content.indexOf("yiqixiu.tongkuvip.com科技公司") > 0 && (f.content = f.content.replace(/yiqixiu.tongkuvip.com科技公司/, '<a href="' + PREFIX_URL + "eqs/link?id=" + a.obj.id + "&amp;url=" + encodeURIComponent(a.obj.property.bottomLabel.url) + '" target=_blank>' + a.obj.property.bottomLabel.name + "</a>"));
						else if (a.obj.property.bottomLabel.name) 2 == f.type && f.content.indexOf("yiqixiu.tongkuvip.com科技公司") > 0 && (f.content = f.content.replace(/yiqixiu.tongkuvip.com科技公司/, a.obj.property.bottomLabel.name));
						else if (/酷秀技术支持/.test(f.content)) {
			...
			type: "GET",
			url: PREFIX_URL+"/media/mobile/assets/vendor/mygrow/json/bg.json",
			xhrFields: {
				withCredentials: !0
			},
			crossDomain: !0,
  ...
  // url= "U6052058SE89";
	...
	if (param.indexOf("preview=preview") > 0 && (preview = !0), param.indexOf("branchid=") > 0 && (branchid = param.substring(param.indexOf("branchid=") + 9)), param.indexOf("mobileview=mobileview") > 0 && (mobileview = !0), isWeixin() && bindWeixin(), !mobilecheck()) {
		var getBg = function() {
              ...
              //      url:"/sf-web/media/mobile/assets/vendor/mygrow/json/cy.json",
                //    url:"/sf-web/mobile/getGrowJson",
			 		xhrFields: {
						withCredentials: !0
					},
					crossDomain: !0
