    ...
    // 方形节点
    function newNode(x, y, w, h, text ,name){
    ...
    function newCircleNode(x, y, w, h, text,name,color,big){
        ...
        var msgNode = new JTopo.TextNode(name);
        msgNode.zIndex++;
        msgNode.setBound(x+40, y+20);
        if (name.length < 4){
            if (big == 'big'){
                msgNode.setBound(x+70, y+20);
            }else{
                msgNode.setBound(x+40, y+20);
            }
        }else{
            if (big == 'big'){
                msgNode.setBound(x+55, y+20);
            }else{
    ...
    var mainNode = newNode(450,300, 100, 100,person[0],"用户姓名");
