    ...
    function  saveOrUpdateForm(){
        ...
        if(!isNullOrEmpty($("input[name='fixDate1']").val())&&!isNullOrEmpty($("input[name='fixDate2']").val())){
            struct+='{fact_attribute:"dayOfMonth","operator":"[]","expectation_value":"'+$("input[name='fixDate1']").val()+','+$("input[name='fixDate2']").val()+'","fact_obj":"com.shufensoft.crm.ruleEngine.infoUnit.system.RuleDate","initFlag":false,"demo":"活动提现日","condition_remind":"亲,请在规定时间内提现"}';
        }
        if(!isNullOrEmpty($("input[name='maxCountPerMon']").val())&&$("input[name='maxCountPerMon']").val()!=0){
            struct+='{fact_attribute:"cachCountPerMonth","operator":"[]","expectation_value":"0,'+$("input[name='maxCountPerMon']").val()+'","fact_obj":"com.shufensoft.crm.biz.vo.UserVO","initFlag":false,"demo":"用户每月提现数量","condition_remind":"sorry,当月您已超出提现次数,期待下次光临"}';
        }
